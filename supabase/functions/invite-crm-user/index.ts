...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Use the caller's session to enforce CRM administrator access.
    const userClient = createClient(supabaseUrl, anonKey, {
      global: { headers: { Authorization: authHeader } },
    });

    const { data: allowed, error: allowedError } =
      await userClient.rpc("is_crm_admin");
    ...
    if (!["admin", "manager", "staff"].includes(role)) {
      throw new Error("Invalid CRM role.");
    }
    ...
    if (rowError) {
      // Avoid leaving an Auth account without a CRM profile if the DB write fails.
